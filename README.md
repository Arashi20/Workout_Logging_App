...
history, weight, discipline, nutrition, steps, journal and PR data to Claude as a
custom connector (and to scripts as a REST API). Journal entries are exposed as
metadata and mood only - the entry text is returned solely when a request asks
for it explicitly. It deploys as its own Railway service pointing at that
